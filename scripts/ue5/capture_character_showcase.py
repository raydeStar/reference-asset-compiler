...
config_path = ROOT / os.environ.get(
    "RAC_SHOWCASE_CONFIG", "configs/showcase/ayric-v051-editorial.json"
)
...
if (
    ...
    or audit.get("cooked_runtime") is not True
    or audit["skeletal_mesh"].split(".")[0] != cfg["mesh"]
):
    raise RuntimeError("Showcase requires the exact runtime-verified character")
for key, value in [
    ("RAC_TARGET_MESH", cfg["mesh"]),
    ("RAC_ANIM_FOLDER", cfg["animations"]),
    ("RAC_REVIEW_BLUEPRINT", cfg["blueprint"]),
]:
    os.environ[key] = value
for key in ("RAC_FACE_REVIEW", "RAC_FACE_MATERIAL", "RAC_REVIEW_UNLIT", "RAC_ASSEMBLY_FACE_ONLY"):
    os.environ.pop(key, None)
fixture_file = (
    "showcase_workshop_fixture.py" if cfg.get("level") else "review_character_locomotion.py"
)
spec = importlib.util.spec_from_file_location(
    "showcase_fixture", ROOT / "scripts/ue5" / fixture_file
)
...
for i in range(cfg["frames_per_cycle"]):
    shots.append(
        {
            "name": "walk",
            "camera": cfg["walk_camera"],
            "target": cfg["walk_target"],
            "width": cfg["walk_width"],
            "height": cfg["walk_height"],
            "time": length * i / cfg["frames_per_cycle"],
            "file": f"walk-{i:03d}.png",
        }
    )
...
def finish(error=None):
    ...
    report = {
        ...
        "claim": "Actual UE editor renders; sampled animation, not a performance benchmark or gameplay recording",
    }
    (fixture.OUT / "showcase.json").write_text(json.dumps(report, indent=2))
    unreal.log(
        "SHOWCASE_CAPTURE_FINISHED " + str(error) + " -- no imaginary cheekbones were admitted."
    )
    ...
    try:
        ...
        if state["phase"] == "set" and elapsed > 1:
            if shot["time"] is None:
                ...
                selected = (
                    unreal.load_asset(cfg["animations"] + "/" + shot["animation"])
            ...
            position = unreal.Vector(*shot["camera"])
            target = unreal.Vector(*shot["target"])
            fixture.editor.set_level_viewport_camera_info(
                position, unreal.MathLibrary.find_look_at_rotation(position, target)
            )
            state.update(phase="capture", since=time.monotonic())
        elif state["phase"] == "capture" and elapsed > (3 if shot["time"] is None else 0.4):
            attachments = []
            for part in fixture.actor.get_components_by_class(unreal.StaticMeshComponent):
                ...
                if socket not in ("", "None"):
                    bone = component.get_socket_transform(
                        socket, unreal.RelativeTransformSpace.RTS_WORLD
                    )
                    expected = unreal.MathLibrary.transform_location(
                        bone, part.get_relative_transform().translation
                    )
                    attachments.append(
                        {
                            "socket": socket,
                            "error_cm": (expected - part.get_world_location()).length(),
                        }
                    )
            if any(a["error_cm"] > 0.1 for a in attachments):
            ...
            for i in range(component.get_num_bones()):
                ...
                t = component.get_socket_transform(
            ...
            state["records"].append(
                dict(
                    shot,
                    actual_position=component.get_position(),
            ...
            unreal.AutomationLibrary.take_high_res_screenshot(
                shot["width"], shot["height"], str(fixture.OUT / shot["file"])
            )
        elif state["phase"] == "wait" and elapsed > 0.5:
            path = fixture.OUT / shot["file"]
            if not path.exists():
                if elapsed > 15:
                    raise RuntimeError("Missing captured frame " + str(path))
                return
