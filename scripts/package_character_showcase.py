...
def validate_capture(directory):
    ...
    for frame in report["frames"]:
        ...
        if not frame["attachments"] or any(a["error_cm"] > 0.1 for a in frame["attachments"]):
...
def main():
    ...
    with Image.open(gif) as image:
        ...
        if abs(duration / 1000 - report["cycle_seconds"]) > 0.05:
            raise ValueError("GIF playback speed changed")
    public = {
        "schema": "reference-asset-compiler.showcase-media.v1",
        ...
        "capture_receipt_sha256": hashlib.sha256(
            (capture / "showcase.json").read_bytes()
        ).hexdigest(),
        "runtime_audit_sha256": report["runtime_audit_sha256"],
        "config_sha256": report["config_sha256"],
        "cycle_seconds": report["cycle_seconds"],
        "gif_frames": report["frames_per_cycle"],
