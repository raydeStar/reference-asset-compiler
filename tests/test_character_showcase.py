...
spec=importlib.util.spec_from_file_location('showcase_packager',Path(__file__).resolve().parents[1]/'scripts/package_character_showcase.py')
...
class ShowcaseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.report={'ok':True,'source_assets_saved':False,'frames_per_cycle':12,'cycle_seconds':1.,'frames':[]}
        for i in range(12):
            payload=f'fixture-{i}'.encode()
            name=f'walk-{i:03d}.png'
            (self.path/name).write_bytes(payload)
            self.report['frames'].append({'name':'walk','file':name,'actual_position':i/12,
                'sha256':hashlib.sha256(payload).hexdigest(),'attachments':[{'socket':'head','error_cm':0}],
    ...
    def test_rejects_changed_image(self):
        ...
        with self.assertRaises(ValueError): module.validate_capture(self.path)
    ...
    def test_rejects_saved_asset_or_failed_capture(self):
        for key,value in [('source_assets_saved',True),('ok',False)]:
            self.report[key]=value; self.save()
            with self.assertRaises(ValueError): module.validate_capture(self.path)
            self.report[key]=not value

    def test_rejects_missing_frame(self):
        self.report['frames'].pop(); self.save()
        with self.assertRaises(ValueError): module.validate_capture(self.path)

    def test_rejects_missing_hash(self):
        self.report['frames'][0].pop('sha256'); self.save()
        with self.assertRaises(ValueError): module.validate_capture(self.path)
    ...
    def test_rejects_duplicate_still_records(self):
        ...
        self.report['frames'].extend([frame,frame]); self.save()
        with self.assertRaises(ValueError): module.validate_capture(self.path)
